""" Codes here are still very primitive
"""
import os
...
sys.path.append("/home/foucault/projects/iNPH/source")
import loggy
...
#from tensorflow.keras import backend as K,activationsfrom tensorflow
...
class ConvNetAqueduct(object):
    def __init__(self):
        ...
        self.logger = logger
        self.debug_level = 0
        self.d_results = Path("/home/foucault/projects/iNPH/results")
    ...
    def debug_input_eval(self):
        with tf.compat.v1.Session() as sess:
            sess.run(tf.compat.v1.global_variables_initializer())
            sess.run(self.train_init)
            img = sess.run(self.img)
    ...
    def debug_eval_eval(self):
        """ try tf.Print """
        with tf.compat.v1.Session() as sess:
            sess.run(tf.compat.v1.global_variables_initializer())
            sess.run(self.train_init)
            sess.run(self.test_init)
            _pred = sess.run(self.preds)
            _label = sess.run(self.label)
            _correct_preds = sess.run(self.correct_preds)
            __import__('IPython').embed()
            sys.exit()
...
class Conv2Branch(ConvNetAqueduct):
    def __init__(self):
        ...
        self.best_acc = 0
        self.best_f1score = 0
        self.d_intermediate = Path("/home/foucault/projects/iNPH/results/intermediate")
...
class CV10(object):
    def __init__(self):
        self.d_intermediate = Path("/home/foucault/projects/iNPH/results/intermediate")
        self.d_csv = Path("/home/foucault/projects/iNPH/results/csv")
        self.n_splits = 10
        self.num_train = None
        self.is_training = True
    ...
    def run_cv(self):
        ...
        fname = self.d_csv.joinpath("aqueduct_csf_acc.csv").as_posix()
...
def test_branches():
    ...
    #model.debug_level = 1
    #model.build()
    #model.get_function_name(model.train)
...
def test_branches_corr():
    aqueduct_model = ConvNetAqueduct()
    #model.debug_level = 1
    #model.build()
    #model.get_function_name(model.train)
    #model.train(n_epochs=3)
...
class Corr_Net(Conv2Branch):
    def __init__(self):
        super().__init__()
        self.aqueduct_img = None
        self.aqueduct_label = None
        self.csf_img = None
        self.csf_label = None
        self.aqueduct_train_init = None
        self.aqueduct_test_init = None
        self.csf_train_init = None
        self.csf_test_init = None


    def get_data(self):
        with tf.compat.v1.name_scope('aqueduct_data'):
            aqueduct_model = ConvNetAqueduct()
            aqueduct_model.get_data()
            self.aqueduct_train_init = aqueduct_model.train_init
            self.aqueduct_test_init = aqueduct_model.test_init
            self.aqueduct_img = aqueduct_model.img
            self.aqueduct_label = aqueduct_model.label

        with tf.compat.v1.name_scope('csf_data'):
            csf_model = ConvNetCSF()
            csf_model.get_data()
            self.csf_train_init = csf_model.train_init
            self.csf_test_init = csf_model.test_init
            self.csf_img = csf_model.img
            self.csf_label = csf_model.label


    def conv(self, inputs, filters, k_size, stride, padding, scope_name):
        '''
        A method that does convolution
        '''
        with tf.compat.v1.variable_scope(scope_name, reuse=tf.compat.v1.AUTO_REUSE) as scope:
            in_channels = inputs.shape[-1]
            kernel = tf.compat.v1.get_variable('kernel',
    ...
    def deconv(self, inputs, filters, k_size, stride, padding, scope_name):
        '''
        A method that does deconvolution
        '''
        with tf.compat.v1.variable_scope(scope_name, reuse=tf.compat.v1.AUTO_REUSE) as scope:
            in_channels = inputs.shape[-1]
            kernel = tf.compat.v1.get_variable('kernel',
    ...
    def inference(self):
        aqueduct_conv1 = self.conv(inputs=self.aqueduct_img,
                                   filters=16,
                                   k_size=8,
                                   stride=1,
                                   padding='SAME',
                                   scope_name='aqueduct_conv1')
        #pool1 = maxpool(conv1, 2, 2, 'VALID', 'pool1')
        csf_conv1 = self.conv(inputs=self.csf_img,
                              filters=16,
                              k_size=8,
                              stride=1,
                              padding='SAME',
                              scope_name='csf_conv1')
        self.H = tf.math.add(aqueduct_conv1, csf_conv1)
        aqueduct_conv2 = self.deconv(inputs=h,
                                     filters=16,
                                     k_size=8,
                                     stride=1,
                                     padding='SAME',
                                     scope_name='aqueduct_conv2')
        csf_conv2 = self.deconv(inputs=h,
                                filters=16,
                                k_size=8,
                                stride=1,
                                padding='SAME',
                                scope_name='csf_conv2')
        #pool2 = maxpool(conv2, 2, 2, 'VALID', 'pool2')
        feature_dim = pool2.shape[1] * pool2.shape[2] * pool2.shape[3]
        pool2 = tf.reshape(pool2, [-1, feature_dim])
        fc = fully_connected(pool2, 8, 'fc')
...
class Corr_Net2(Conv2Branch):
    def __init__(self):
        ...
        self.csf_test_init = None
        self.hdim_deep = 50
        self.d_models = Path("/home/foucault/projects/iNPH/results/models")
    ...
    def model_corrnet(self, Ia, Iv, epoch):
        #inputDimx,inputDimy are the dimentions two input modalities. And
        #hdim_deep is the dimentions of shared representation(kept as
        #global veriable)
        inputDimx = Ia.shape[1]
        inputDimy = Iv.shape[1]
        inpx = Input(shape=(inputDimx,))
        inpy = Input(shape=(inputDimy,))
        #Encoder
        hl = Dense(self.hdim_deep, activation='relu')(inpx)
        hr = Dense(self.hdim_deep, activation='relu')(inpy)
        h =  Add()([hl,hr]) #Common representation/Encoded representation
        #decoder
        ...
        As during training we have to take gradient from 3 different loss
        function and which can not be obtained from signle input.
        If you look closely to the loss function, we will see it has different
        input parameter'''''
        ...
        Lambda = 0.01
        #by convention error function need to have two argument, but in
        #correlation loss we do not need any argument. Loss is computed
        #competely based on correlation of two hidden representation. For
        #this a 'fake' argument is used.
        ...
        model.compile(
            loss=[square_loss,square_loss,square_loss,square_loss,square_loss,square_loss,correlationLoss],
            optimizer="adam")
        #model.summary()
        '''
        Suppose you have already prepared your data and kept one moadlity data in
        Ia(e.g. Audio) and another in Iv( e.g. Video ).To be used by this model
        Audios and videos must be converted into 1D tensor.abs'''
        model.fit([Ia,Iv],[Ia,Ia,Ia,Iv,Iv,Iv,np.ones((Ia.shape[0],Ia.shape[1]))],nb_epoch=epoch)
        '''
        np.ones((Ia.shape[0],Ia.shape[1])) is fake tensor that will be
        passed to correlationLoss function but will have no use
        '''
    ...
    def get_data(self):
        ...
        self.n_test = dc.dict_n_subject[dataset_key] - self.num_train
        self.logger.info(f"num_train: {self.num_train}")
        a_train_np, a_val_np = dc.get_adueduct_data_ml_train(self.num_train)
        c_train_np, c_val_np = dc.get_csf_data_ml_train(self.num_train)
        ia = a_train_np[0].reshape(-1, 17*14*32)
        iv = c_train_np[0].reshape(-1, 313*433*10)
        ia_val = a_val_np[0].reshape(-1, 17*14*32)
        iv_val = c_val_np[0].reshape(-1, 313*433*10)
        return ia, iv, a_train_np[1], ia_val, iv_val, a_val_np[1]


    def train_model(self):
        (Ia, Iv, self.train_labels, Ia_val, Iv_val,
         self.val_labels) = self.get_data()
        epoch = 1
        corr_net = self.model_corrnet(Ia, Iv, epoch)
        _, _, h = corr_net.predict([Ia, Iv])
        _, _, h_val = corr_net.predict([Ia_val, Iv_val])
        return h, h_val


    def svm_clf(self, X_train, y_train):
    ...
    def rf_clf(self, X_train, y_train):
    ...
    def xgb_clf(self, X_train, y_train):
    ...
    def classfier(self):
        ...
        h, h_val = self.train_model()
        X_train = h
        X_test = h_val
        y_train = np.argmax(self.train_labels, axis=1)
        y_test = np.argmax(self.val_labels, axis=1)
        print(self.train_labels)
        print(self.val_labels)
        #_classifier = self.svm_clf(X_train, y_train)
        #_classifier = self.rf_clf(X_train, y_train)
        _classifier = self.xgb_clf(X_train, y_train)
...
class CV10_corrnet(CV10):
    ...
    def run_cv(self):
        ...
        for fold_id, indices in enumerate(indices_list):
            ...
            acc = model.classfier()
            self.cv_acc_list.append(acc)
        self.logger.info(self.cv_acc_list)
        df = pd.DataFrame(data=self.cv_acc_list).T
        df["mean"] = np.mean(self.cv_acc_list, dtype=np.float32)
        fname = self.d_csv.joinpath("corrnet_xgb.csv").as_posix()
        #fname = self.d_csv.joinpath("aqueduct_csf_acc_normalized_input.csv").as_posix()
        check_dir_or_mkdir(self, self.d_csv)
        write_df2csv(self, df, fname)
